...
export default async function Dashboard({ searchParams }: { searchParams: Promise<{ convite?: string; revisao?: string }> }) {
  ...
  return <div className="flex flex-col gap-5">
    ...
    <section aria-labelledby="attention-title"><SectionTitle id="attention-title" title="Precisa de você" detail="Só mostramos o que pede uma decisão." /><div className="action-list mt-2">
    ...
    {upcoming.length > 0 && <section className="card" aria-labelledby="upcoming-title"><SectionTitle id="upcoming-title" title="Próximos 30 dias" detail="Parcelas confirmadas e padrões estimados." action={{ href: "/recorrencias", label: "Ver tudo" }} /><div className="timeline mt-3">{upcoming.map((item) => <div className="timeline-row" key={item.id}><time dateTime={item.date} className="timeline-date">{shortDate(item.date)}</time><div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate">{item.label}</p><p className="text-xs text-muted">{item.detail}</p></div><strong className="text-sm whitespace-nowrap">{brl(item.amount)}</strong></div>)}</div></section>}

    {isPersonal && income > 0 && <section className="card" aria-labelledby="plan-title"><SectionTitle id="plan-title" title="Seu plano" detail="Limites são orientação, não julgamento." action={{ href: "/perfil?secao=planejamento", label: "Ajustar" }} /><div className="flex flex-col gap-3 mt-3"><PlanRow label="Obrigatórias" value={bucketTotal("obrigatoria")} target={income * ruleOf("obrigatoria")} /><PlanRow label="Flexíveis" value={bucketTotal("nao_obrig")} target={income * ruleOf("nao_obrig")} /><PlanRow label="Investimentos" value={investedMonth} target={income * ruleOf("investimento")} reverse /></div>{previousSpent > 0 && <p className="text-xs text-muted mt-3">Este mês está {trendLabel(spent, previousSpent)} em relação ao mês anterior.</p>}</section>}

    {activeGoals.length > 0 && <section className="card" aria-labelledby="goals-title"><SectionTitle id="goals-title" title="Metas em movimento" detail={`${brl(saved)} já reservados`} action={{ href: "/caixinhas", label: "Gerenciar" }} /><div className="flex flex-col gap-4 mt-3">{activeGoals.slice(0, 3).map((goal) => { const need = monthlyGoalNeed(Number(goal.current_amount), Number(goal.target_amount), goal.deadline, now); return <div key={goal.id}><div className="flex justify-between gap-3 text-sm"><strong>{goal.name}</strong><span className="text-muted whitespace-nowrap">{pct(goal.progresso)}</span></div><div className="bar mt-2"><span style={{ width: `${Math.min(100, Number(goal.progresso) * 100)}%` }} /></div><p className="text-xs text-muted mt-1">{brl(goal.current_amount)} de {brl(goal.target_amount)}{need ? ` · reserve ${brl(need)}/mês para chegar no prazo` : ""}</p></div>; })}</div></section>}

    {isHouse && <section className="card" aria-labelledby="house-title"><SectionTitle id="house-title" title="Casa" detail="Visão compartilhada sem misturar contas pessoais." action={{ href: "/casa/compras", label: "Abrir" }} /><dl className="metric-strip mt-3"><Metric label="Custo mensal previsto" value={houseMonthly} /><Metric label="Compras realizadas" value={houseBought} /><Metric label="Acertos pendentes" value={owed + receivable} /></dl></section>}
  </div>;
}
