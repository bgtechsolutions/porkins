...
export function BottomNav({ showHouse }: { showHouse: boolean }) {
  const pathname = usePathname();
  const items = showHouse
    ? [...BASE_ITEMS.slice(0, 4), { href: "/casa/compras", label: "Casa", icon: "home" as const }, BASE_ITEMS[4]]
    : [...BASE_ITEMS];
  ...
  return (
    <nav className="bottom-nav fixed bottom-0 inset-x-0 border-t pb-[env(safe-area-inset-bottom)] z-20" aria-label="Navegação principal">
      <div className={`mx-auto max-w-lg grid ${items.length === 6 ? "grid-cols-6" : "grid-cols-5"}`}>
        {items.map((item) => {
          const active = item.href.startsWith("/casa/")
...
function NavIcon({ name }: { name: "chart" | "plus" | "receipt" | "users" | "home" | "user" }) {
